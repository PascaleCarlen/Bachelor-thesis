clear all
pname = '/Volumes/NeueFestplatte/Snowhorizons1/';
...
for i = 1:length(file)
    ...
    z_hor1 = [z_hor1;t.Var5]; %depth below surface of Horizon 1 
...
scatter(x,y,10,z_hor1) %10: Dicke der Linien und Farben sind nach z_hor1
colorbar


%Remove 0m depth from x,y,z_hor1 
ox=x;
oy=y;
oz_hor1=z_hor1;
x=x(z_hor1~=0);
y=y(z_hor1~=0);
z_surf=z_surf(z_hor1~=0);
z_hor1=z_hor1(z_hor1~=0);
figure;grid on;hold on;
scatter(x,y,10,z_hor1)
colorbar

elev_hor = 2250 - z_surf + z_hor1; %damit das Datum passt


%Only run once to pick polygon und dann auskommentieren und load('Horizon1.mat')
% [x1,y1]=getpts
% save('Hor1.mat','x1','y1')
...
save('sm_hor1.txt','ascii','-ascii')
%save('Oberaletsch_Hor1_interpolation.mat')